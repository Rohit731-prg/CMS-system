...
function Customers() {
  ...
  return (
    <main className="flex min-h-screen font-inter bg-gray-100">
      <Sidebar />

      <div className="flex-1">
        {/* Navbar */}
        <nav className="flex justify-end items-center gap-4 bg-white px-6 py-4 shadow">
          <img
            src={user?.image}
            alt="User"
        ...
        {customers ? (
          <div className="flex flex-col md:flex-row gap-6 p-6">
            {/* Customer List */}
            <section className="md:w-[40%] bg-white rounded-lg shadow p-6 max-h-[75vh] overflow-y-auto">
                ...
                {filteredCustomers.length > 0 ? (
                  filteredCustomers.map((cus) => (
                    <div
                      key={cus._id}
                      onClick={() =>
                        useCustomerStore.getState().setCustoer(cus)
                      }
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-600">{cus.email}</p>
            ...
            {/* Customer Details */}
            <section className="md:w-[60%] bg-white rounded-lg shadow p-6">
              ...
              {customer ? (
                <div className="space-y-3 text-sm text-gray-700">
                  {[
                    ...
                    >
                      <span className="text-gray-500 font-medium w-1/3 capitalize">
                        {key.replace("_", " ")}
                      </span>
                      <span className="text-right w-2/3">{customer[key]}</span>
                    </div>
                  ))}
                  <div className="flex justify-between">
                    <button
                      ...
                      className="mt-4 flex items-center gap-2 bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition"
                    >
                    ...
                    {!customer.status && (
                      <button
                      className="mt-4 flex items-center gap-2 bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 transition"
                      onClick={() => useCustomerStore.getState().updateCustomer(customer._id)}
                      >
                        Mark as Read
                      </button>
                    )}
